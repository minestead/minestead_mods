...
local function bike_anim(self)
	...
	if self.driver then
		...
		-- Wheely
		if ctrl.jump or self.object:get_velocity().y > 0 then
			-- We are moving
			if self.v > 0 then
...
function bike.on_step(self, dtime)
	...
	if self.driver then
		...
		if self.may_jump and ctrl.jump and self.object:get_velocity().y == 0 then
			local vel = get_velocity(self.v, self.object:get_yaw(), 3.7)
			self.object:set_velocity(vel)
			self.may_jump = false
		end

		local turn_speed = 2
		if self.v >= self.max_v and not ctrl.jump then
			turn_speed = 1
		end
