...
minetest.register_entity("hangglider:glider", {
	...
	on_step = function(self, dtime)
		...
		if self.object:get_attach() then
			...
			local k = perlin_map:get3d{x = pos.x, y = pos.z, z = pos.y}
					...
					if mrn_name then
						if not (mrn_name.walkable or mrn_name.liquidtype ~= "none") then
							...
							if not minetestd then
								...
								local g = ((3 + step_v) / 20) * k
								if v < 2 then g = 1
								elseif v >= 2 and step_v <= -7 then 
									g = -0.3
								end
								if step_v > 3 and g < 0 then
									g = -g
								end
								apply_physics_override(player, {gravity= g})
							end
...
minetest.register_tool("hangglider:hangglider", {
	...
	on_use = function(itemstack, player, pointed_thing)
		...
		if not perlin_map then
			perlin_map = minetest.get_perlin(np_density)
		end
		local pos = player:get_pos()
		local pname = player:get_player_name()
