...
local function spawnstep(dtime)
	...
	if timer > waterinterval then
		...
		for _,plyr in ipairs(minetest.get_connected_players()) do
			...
			if plyr and plyr:is_player() and plyr:get_pos().y > -50 and plyr:get_pos().y < 150 and plyr:get_pos().z < -4000 and not water_life.apionly then	-- each player gets a spawn chance every 10s on average
				...
				if liquidflag and not toomuch and surface then
					...
					mobname = 'water_life:fish'
						...
						if water_life.fish_spawn_rate >= random(1000) and ((animal.all < (water_life.maxmobs-5)) or getcount(animal[mobname]) < 5) and (((liquidflag == "sea" or liquidflag == "river" or liquidflag == "muddy")) or (water_life.spawn_on_islands and not water_life.check_for_pool(nil,2,3,pos2) and
